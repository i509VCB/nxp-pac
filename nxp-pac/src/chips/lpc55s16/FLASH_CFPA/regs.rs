...
impl defmt::Format for DCFG_CC_SOCU_DFLT {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "DCFG_CC_SOCU_DFLT {{ NIDEN: {=bool:?}, DBGEN: {=bool:?}, SPNIDEN: {=bool:?}, SPIDEN: {=bool:?}, TAPEN: {=bool:?}, ISP_CMD_EN: {=bool:?}, FA_ME_CMD_EN: {=bool:?}, INVERSE_VALUE: {=u16:?} }}",
            self.NIDEN(),
            self.DBGEN(),
            self.SPNIDEN(),
            self.SPIDEN(),
...
impl DCFG_CC_SOCU_PIN {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn FA_ME_CMD_EN(&self) -> super::vals::DCFG_CC_SOCU_PIN_FA_ME_CMD_EN {
        let val = (self.0 >> 7usize) & 0x01;
        super::vals::DCFG_CC_SOCU_PIN_FA_ME_CMD_EN::from_bits(val as u8)
    }
    #[doc = "FA Command enable."]
    #[inline(always)]
    pub const fn set_FA_ME_CMD_EN(&mut self, val: super::vals::DCFG_CC_SOCU_PIN_FA_ME_CMD_EN) {
...
impl defmt::Format for DCFG_CC_SOCU_PIN {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "DCFG_CC_SOCU_PIN {{ NIDEN: {:?}, DBGEN: {:?}, SPNIDEN: {:?}, SPIDEN: {:?}, TAPEN: {:?}, ISP_CMD_EN: {:?}, FA_ME_CMD_EN: {:?}, UUID_CHECK: {=bool:?}, INVERSE_VALUE: {=u16:?} }}",
            self.NIDEN(),
            self.DBGEN(),
            self.SPNIDEN(),
            self.SPIDEN(),
