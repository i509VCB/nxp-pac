...
impl CST_LONG {
    #[inline(always)]
    pub const fn from_bits(val: u8) -> CST_LONG {
        unsafe { core::mem::transmute(val & 0x01) }
    }
    #[inline(always)]
    pub const fn to_bits(self) -> u8 {
...
impl From<u8> for CST_LONG {
    #[inline(always)]
    fn from(val: u8) -> CST_LONG {
        CST_LONG::from_bits(val)
    }
}
impl From<CST_LONG> for u8 {
    #[inline(always)]
    fn from(val: CST_LONG) -> u8 {
        CST_LONG::to_bits(val)
    }
}
#[repr(u8)]
#[derive(Copy, Clone, Debug, Eq, PartialEq, Ord, PartialOrd)]
